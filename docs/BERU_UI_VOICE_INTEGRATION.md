# Beru UI ↔ Backend (voice + chat)

**API:** `http://localhost:8000` — start with `python3 beru_api.py` from the `beru` repo.
...
| Backend (`src/text_direction.py`) | beru-ui |
|-----------------------------------|---------|
...
| `src/hooks/useChat.ts` | Stores `text_direction` from API on Beru messages |
